    ...
    def parse_json(self,filename):
        ...
        with open(filename, encoding='utf-8') as json_file:
            ...
            for set in data:
                ...
                rules = [self.Rule(self.toProposition(rules_data[rule]["antecedence"].encode('utf-8').decode('utf-8')), self.toProposition(rules_data[rule]["consequence"].encode('utf-8').decode('utf-8'))) for rule in rules_data]

                for fact_data in facts_data:
                    facts.append(self.toProposition(fact_data))

                facts = [self.toProposition(fact_data) for fact_data in facts_data]
